...
func new_game():
	...
	game_running = true
	steps = [0, 0, 0]
	$CanvasLayer.get_node("Label3").hide()
	clear_piece()
	clear_board()
	clear_panel()
...
func create_piece():
	...
	draw_piece(next_piece_type[0], Vector2i(13, 5), next_piece_atlas)
func clear_piece():
...
func check_rows():
	...
	while row > 0:
		...
		if count  == COLS:
			...
			shift_rows(row)
			score += REWARD
			$CanvasLayer.get_node("Label").text = "SCORE: " + str(score)
			speed += ACCEL
		else:
			row -= 1
func shift_rows(row):
...
func check_game_over():
	for i in active_piece:
		if not is_free(i + cur_pos):
			land_piece()
			$CanvasLayer.get_node("Label3").show()
			game_running = false
